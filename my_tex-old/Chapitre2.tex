...
%	On montre dans \cite{Di3} avec cet exemple que toutes les filtrations n'admettent pas de réduction minimale.
%\end{maremarque}
%\begin{monexemple}
%	Soient l'anneau $A = \mathbb{K}\left[ X\right]$ où $\mathbb{K}$ est un corps et $I = (X)$ un idéal de $A$.\\
%	Considérons la filtration définie par $I_{2n} = I_{2n-1} = I^n, \text{ pour tout } n \in \mathbb{N}$.\\
...
%		I_{m+n} = I_m I_n &\text{ d'où } \exists Q \in A,\; QX^{k+p+2} = X^{k+p+1}.\\
